...
class BuildConfig(object):
    """
    Debian package configuration class
    """
    variables = {
    ...
    @staticmethod
    def pack_sourcecode(path_name, targz_name):
        """
        Create initial source tarball
        """
    ...
    def prepare_sourcecode(self):
        """
        Do prepare operations:
        1. Clone from git
        2. Set new package name
        3. Pack source tarball
        4. Build package source
        """
        ...
        logging.info('Cloning repo %s into %s', repr(self.product['git']), repr(path_to))
        try:
            ...
            logging.error(
                'Failed cloning repo %s with error %s',
                repr(self.product['git']), e.stderr
            )
            logging.error('Aborting.')
            exit(1)
        ...
        try:
            ...
            logging.error(
                ...
                repr(self.product['branch']), e.stderr
            )
            logging.error('Aborting.')
            exit(1)
    ...
    def import_package(self, repo_path):
        """
        Move package to the repository using reprepro
        """
        path_to = os.path.join(*self.variables['resultdir']).format(
            distro=self.distro['distro'],
            arch=self.arch,
